...
function buildWhatsAppMessage(extra = {}) {
    ...
    if (extra.hora) lines.push(`Hora: ${extra.hora}`);
...
function reservationItemTemplate(r) {
    const nombreCliente = r.nombreCliente ? `<p class="reservation-client-name">${escapeHtml(r.nombreCliente)}</p>` : '';
    const cumpleanos = r.fechaCumpleanos ? `<p class="reservation-item-secondary">Cumpleaños: ${escapeHtml(r.fechaCumpleanos)}</p>` : '';
    const stage = r.crmStage ? `<span class="reservation-stage-badge">${escapeHtml(r.crmStage)}</span>` : '';

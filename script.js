...
function getSelectedYateId() {
    const value = document.getElementById('select-yate')?.value || '';
    const parsed = parseInt(value, 10);
    return Number.isNaN(parsed) ? null : parsed;
}

function getSelectedYate() {
    const yateId = getSelectedYateId();
    return yates.find(y => Number(y.id) === Number(yateId)) || null;
}

function getReservaEmbarcacionValue() {
    const typedValue = (document.getElementById('reserva-embarcacion')?.value || '').trim();
    const selectedYate = getSelectedYate();
    return typedValue || selectedYate?.nombre || '';
}

function getReservaKey(value) {
    return normalizeText(value);
}

function syncReservaTextboxWithSelect(force = false) {
    const input = document.getElementById('reserva-embarcacion');
    if (!input) return;

    const selectedYate = getSelectedYate();
    if (selectedYate && (force || !input.value.trim())) {
        input.value = selectedYate.nombre;
    }
}

function handleReservaTextboxInput() {
    const input = document.getElementById('reserva-embarcacion');
    const selectedYate = getSelectedYate();
    const typedValue = (input?.value || '').trim();

    if (typedValue && selectedYate && getReservaKey(typedValue) !== getReservaKey(selectedYate.nombre)) {
...
function reservationMatchesResource(reserva, yateId, reservaNombre) {
    const reservaKey = getReservaKey(reservaNombre);
    const storedName = reserva?.reserva || reserva?.embarcacionReservada || reserva?.yate || '';
    const sameYateId = yateId !== null && yateId !== undefined && Number(reserva?.yateId) === Number(yateId);
    const sameName = reservaKey && getReservaKey(storedName) === reservaKey;
    return sameYateId || sameName;
}

function obtenerHorasReservadas(yateId, fecha, reservaNombre = '') {
    if (!fecha) return [];
    return reservas
        .filter(r => r.fecha === fecha && reservationMatchesResource(r, yateId, reservaNombre))
        .map(r => r.hora);
}

function actualizarDisponibilidad() {
    const yateId = getSelectedYateId();
    const reservaNombre = getReservaEmbarcacionValue();
    ...
    const horasReservadas = obtenerHorasReservadas(yateId, fecha, reservaNombre);
    ...
    if (!reservaNombre || !fecha) {
        availability.textContent = 'Selecciona o escribe una embarcación y una fecha para ver horarios disponibles.';
        renderHeroPanel();
        return;
    }
...
async function realizarReserva() {
    ...
    const yateId = getSelectedYateId();
    const yate = getSelectedYate();
    const reservaEmbarcacion = getReservaEmbarcacionValue();
    const reservaKey = getReservaKey(reservaEmbarcacion);
    const fecha = document.getElementById('fecha').value;
    ...
    if (!reservaEmbarcacion || !fecha || !hora) {
        showToast('Completa la embarcación, fecha y hora antes de confirmar.', 'error');
        return;
    }
    ...
    try {
        ...
        const horarioOcupado = snapshot.docs.some(doc => {
            const reserva = doc.data();
            return reservationMatchesResource(reserva, yateId, reservaEmbarcacion);
        });

        if (horarioOcupado) {
            showToast('Ese horario ya fue reservado para esa embarcación. Elige otro.', 'error');
        ...
        const payload = {
            ...
            telefono,
            fechaCumpleanos,
            reserva: reservaEmbarcacion,
            embarcacionReservada: reservaEmbarcacion,
            reservaKey,
            yate: reservaEmbarcacion,
            yateId,
            yateCatalogo: yate ? {
                id: yate.id,
                nombre: yate.nombre,
            ...
            source: 'website',
            tracking: trackingSnapshot,
            whatsappLink: getWhatsAppLink({ nombreCliente, telefono, yate: reservaEmbarcacion, fecha, hora }),
        ...
        trackLeadEvent({
            value: 1,
            currency: 'MXN',
            item_name: reservaEmbarcacion
        });

        hydrateWhatsAppLinks();
        document.getElementById('modal-whatsapp-link')?.setAttribute('href', payload.whatsappLink);

        showToast(`Reserva confirmada para ${reservaEmbarcacion} a las ${formatHourLabel(hora)}.`, 'success');
...
function seleccionarYate(id) {
    ...
    if (select) {
        ...
        syncReservaTextboxWithSelect(true);
        actualizarDisponibilidad();
    }
}
...
function llenarSelectorYates() {
    ...
    syncReservaTextboxWithSelect(false);
}
...
function limpiarFormularioReserva() {
    ...
    const select = document.getElementById('select-yate');
    const reservaInput = document.getElementById('reserva-embarcacion');
    ...
    syncReservaTextboxWithSelect(Boolean(yateSeleccionado));
...
function setupInteractions() {
    ...
    document.getElementById('reserva-embarcacion')?.addEventListener('input', handleReservaTextboxInput);

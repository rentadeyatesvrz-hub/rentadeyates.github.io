...
function renderFlota() {
    ...
    yates.forEach(y => {
        ...
        const incluyeChips = incluyeContenido
            ? incluyeContenido.split(',').map(item => item.trim()).filter(Boolean).slice(0, 6).map(item => `<span class="include-chip">${item}</span>`).join('')
            : '';
        ...
        card.innerHTML = `
            <div class="fleet-media h-72">
                <span class="fleet-tag">${y.tipo}</span>
                <img src="${y.img}" alt="${y.nombre}" class="w-full h-full object-cover">
            </div>
            <div class="p-5 sm:p-6">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h3 class="text-2xl font-bold tracking-[-0.03em]">${y.nombre}</h3>
                        <p class="text-slate-300 mt-2">${y.tipo} • ${y.capacidad}</p>
                    </div>
                    <span class="fleet-badge">Elite</span>
                </div>

                <p class="text-3xl font-bold text-amber-300 mt-5">${y.precio}</p>

                <div class="fleet-meta">
                    <div class="fleet-meta-item">
                        <span class="fleet-meta-label">Categoría</span>
                        <span class="fleet-meta-value">${y.tipo}</span>
                    </div>
                    <div class="fleet-meta-item">
                        <span class="fleet-meta-label">Capacidad</span>
                        <span class="fleet-meta-value">${y.capacidad}</span>
                    </div>
                </div>
...
function mostrarReservasEnLista() {
    ...
    reservas.forEach(r => {
        ...
        div.innerHTML = `
            <div>
                <p class="reservation-item-title">${r.yate}</p>
                <p class="reservation-item-meta">${r.fecha} • ${r.hora}</p>
            </div>
...
function actualizarDisponibilidad() {
    ...
    if (!horaSelect || !availability) return;
